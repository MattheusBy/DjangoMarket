...
from market.forms import ReviewCreateForm, UserCreateForm, \
    FeedbackForm, AddToFavoritesForm
from market.models import Product, Reviews, Category, \
    Stocks, Subcategory, CustomUser, Favorites
from market.parsing_currency import euro, dollar
...
class IndexView(ListView):
    ...
    def get_queryset(self):
        # function to get random 3 product from DB
        # and displays it on index-page in carousel
...
class RegisterUser(CreateView):
    """ Class for register. Shows RegisterForm
    and create new user
    ...
    def form_valid(self, form):
        ...
        data = form.data
        subject = 'Сообщение от DjangoMarket@yandex.by.' \
        ...
        create_city = CustomUser.objects.create(
            city=data["city"], user_for_city=new_user
            )
        create_city.save()
        # send message to register user's mail
...
class ProductView(TemplateView):
    """ View all information about each product:
        price,image,description,reviews.
    ...
    def get(self, request, *args, **kwargs):
        ...
        context["reviews"] = Reviews.objects.filter(
            product=context["product_pk"])
        product = Product.objects.get(
            pk=context["product_pk"])
        context["euro"] = round(
            (product.price / float(euro)
             ), 2)
        context["dollar"] = round(
            (product.price / float(dollar)
             ), 2)
        ...
        if request.user.is_authenticated:
            favorites_products = list(
                Favorites.objects.filter(
                    Q(user=request.user)
                    & Q(product_favorite=context["product_pk"])))
...
class ProductCategoryView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context["products"] = Product.objects.filter(
            category=context["category_pk"])
        context["category"] = Category.objects.get(
            pk=context['category_pk'])
        return self.render_to_response(context)
...
class StocksView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context["count"] = Stocks.objects.all().count()
        return self.render_to_response(context)
...
class StocksInfoView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        context = self.get_context_data(**kwargs)
        context["stock"] = Stocks.objects.get(pk=context["stock_pk"])
        return self.render_to_response(context)
...
class FeedbackView(CreateView):
    """ Class provides possibility to send user's quetions.
        Requires user's email-address, topic.
...
class SearchResultsView(ListView):
    ...
    Shows all products which contains search words
    """
...
class ProductAlreadyFavorites(TemplateView):
    """
    Shows page with information
    if current product already in user's favorites
    """
...
def remove_from_favorites(request, product_favorite_pk):
    ...
    user_favorites = Favorites.objects.filter(
        ...
        product_favorite=product_favorite_pk)
